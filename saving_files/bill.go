...
func (b *bill) saveBill() {
	// when saving data to files, we need to save the data in byte slice format OR a slice of bytes etc
	data := []byte(b.format())
	err := os.WriteFile("bills/"+b.name+".txt", data, 0644)
	if err != nil {
		panic(err) // panic() - this function stops the flow of the program and prints out the error
	}
